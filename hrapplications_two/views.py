...
@login_required
@permission_required('hrapplications_two.access_application')
def hr_application_management_view(request):
    ...
    return render(request, 'hrapplications_two/management.html', context=context)
...
def hr_application_create_view(request, form_id=None):
    if form_id:
        ...
        if request.method == "POST":
            ...
            return redirect('hrapplications_two:personal_view', application.id)
        else:
            questions = app_form.questions.all()
            return render(request, 'hrapplications_two/create.html', context={'questions': questions, 'corp': app_form.corp})
    else:
        choices = []
        ...
        return render(request, 'hrapplications_two/corpchoice.html', context={'choices': choices})
...
def hr_application_personal_view(request, app_id):
    ...
    if app.user == request.user:
        ...
        return render(request, 'hrapplications_two/view.html', context=context)
    else:
        logger.warning(f"User {request.user} not authorized to view {app}")
        return redirect('hrapplications_two:personal_view')
...
def hr_application_personal_removal(request, app_id):
    ...
    return redirect('hrapplications_two:index')
...
def hr_application_view(request, app_id):
    ...
    try:
        ...
        raise Http404
    if request.method == 'POST':
        if request.user.has_perm('hrapplications_two.add_applicationcomment'):
            ...
            if form.is_valid():
                ...
                return redirect('hrapplications_two:view', app_id)
        else:
...
@login_required
@permission_required('auth.human_resources')
@permission_required('hrapplications_two.delete_application')
@permission_required('hrapplications_two.access_application')
...
@login_required
@permission_required('auth.human_resources')
@permission_required('hrapplications_two.approve_application')
@permission_required('hrapplications_two.access_application')
...
def hr_application_search(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return render(request, 'hrapplications_two/searchview.html', context=context)
        else:
        ...
        return redirect("hrapplications_two:view")
